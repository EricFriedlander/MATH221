...
Let's revisit our problem from last time. We were attempting to predict chest diameter (`che_di`) from height (`hgt`) using linear regression. In the console below create a scatter plot with height on the x-axis and chest diameter on the y-axis:
...
In the console below, compute $\bar{x}$, $\bar{y}$, $s_y$, $s_x$, and $r$. The `summarize` coupled with `sd`, `mean`, and `cor` will be useful:

```{r summary-stats, exercise=TRUE}

```

Now, using R as a calculator, compute the least squares $b_0$ and $b_1$:
...
Plug your answers in for `b0` and `b1` below:
...
Now suppose you have a customer walk into your store looking to try on a suit. He says that he is 180 cm tall. Use R below as a calculator to estimate his chest diameter:
