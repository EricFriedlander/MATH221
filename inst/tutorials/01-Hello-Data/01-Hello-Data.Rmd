...
In your group discuss your results. What can we say we about how whether the use of stents prevent strokes?
...
In the console below plot any two variables which are positively associated in the `county` dataset:
...
In the console below plot any two variables which are negatively associated in the `county` dataset:
...
```{r encode, echo=FALSE}
learnrhash::encoder_ui(ui_before = learnrhash::default_ui(url="https://forms.gle/EgGscjvpPDGJJDJZA"))
```
